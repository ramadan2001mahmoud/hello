...
function buildWorkspace() {
    ...
    let html = `
        ...
        ${getToolOptionsHTML()}
        ${currentTool==='signature'?'<div class="sig-box"><canvas id="sigCanvas"></canvas><div class="sig-ctrl"><button class="btn" onclick="clearSig()" style="width:auto;padding:0.5rem;">مسح</button></div></div>':''}
        ${['ocr-image','ocr-pdf'].includes(currentTool)?'<div class="ocr-box" id="ocrOutput"><p style="color:#999;text-align:center;">النص المستخرج...</p></div>':''}
...
function getToolOptionsHTML() {
    switch (currentTool) {
        ...
        case 'protect': return '<div class="options-box"><h4>اعدادات الحماية</h4><div class="opt-row"><span class="opt-label">كلمة المرور:</span><input type="password" id="pp" placeholder="ادخل كلمة مرور قوية"></div><div class="opt-row"><span class="opt-label">تاكيد كلمة المرور:</span><input type="password" id="pp2" placeholder="اعد كتابة كلمة المرور"></div></div>';
        case 'unlock': return '<div class="options-box"><h4>كلمة مرور الملف</h4><input type="password" id="up" placeholder="ادخل كلمة المرور"></div>';
        case 'watermark': return '<div class="options-box"><h4>نص العلامة (English)</h4><input type="text" id="wt" value="PDF Master Pro"><div class="opt-row" style="margin-top:0.5rem;"><span class="opt-label">الحجم:</span><input type="number" id="ws" value="30" min="10" max="80"></div></div>';
        default: return '';
    }
}
...
// FILE HANDLING
// ============================================
function handleFiles(event) { if (event.target.files.length) addFiles(Array.from(event.target.files)); }
...
function openCloudPicker() {
    ...
    inp.type = 'file';
    inp.accept = '.pdf,.doc,.docx,.xls,.xlsx,.ppt,.pptx,image/*';
    inp.multiple = true;
    inp.onchange = e => { if (e.target.files.length) addFiles(Array.from(e.target.files)); };
    inp.click();
}
...
function initSig() {
    ...
    c.height = 200;
    sigPad = new SignaturePad(c, { backgroundColor: 'white', penColor: '#333' });
}

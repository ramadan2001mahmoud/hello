...
async function doWatermark() {
    ...
    try {
        ...
        if (!text || text.trim().length === 0) text = 'PDF Master Pro';
        ...
        showResult('<p>تمت اضافة العلامة المائية</p><p style="color:#666;">النص: ' + text + '</p>', 'watermarked.pdf');
    } catch (e) {
        hideProgress();
        if (e.message && e.message.includes('WinAnsi')) showToast('استخدم حروف انجليزية فقط', 'error');
